...
describe('sheet', () => {
	...
	it('should read sheet by index (first) (default)', async () => {
		const data = await readXlsx(__dirname + '/spreadsheets/multiple-sheets.xlsx')
		expect(data.length).to.equal(1)
		data[0][0].should.equal('First sheet')
	})

	it('should read sheet by index (first)', async () => {
		const data = await readXlsx(__dirname + '/spreadsheets/multiple-sheets.xlsx', { sheet: 1 })
		expect(data.length).to.equal(1)
		data[0][0].should.equal('First sheet')
	})

	it('should read sheet by name (second)', async () => {
		const data = await readXlsx(__dirname + '/spreadsheets/multiple-sheets.xlsx', { sheet: 2 })
		expect(data.length).to.equal(1)
		data[0][0].should.equal('Second sheet')
	})
